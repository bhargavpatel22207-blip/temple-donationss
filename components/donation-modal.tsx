...
import { getRazorpayKeyId } from "@/app/actions/razorpay"
...
export function DonationModal({ isOpen, onClose, onSuccess }: DonationModalProps) {
  ...
  useEffect(() => {
    ...
    // Fetch the Razorpay key from server
    getRazorpayKeyId().then(setRazorpayKey)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const options: RazorpayOptions = {
        ...
        currency: "INR",
        name: "Tatpally Hanuman Mandir",
        description: "Temple Donation",
        handler: async (response: RazorpayResponse) => {
          // Payment successful, generate token and save to database
          try {
            ...
            const { error } = await supabase.from("donations").insert({
              ...
              token: token, // Save token to database
            })
            ...
            setErrors({ submit: "Payment successful but failed to record. Please contact support." })
            setIsLoading(false)
          }
        },
        prefill: {
      ...
      setErrors({ submit: err instanceof Error ? err.message : "Failed to initiate payment. Please try again." })
      setIsLoading(false)
    }
  }
